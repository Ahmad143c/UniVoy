...
import { People, Assignment, Timeline, Chat, Logout } from "@mui/icons-material";
import { useNavigate } from "react-router-dom";
import logo from "../../assets/logo.png";
import sidefavicon from "../../assets/favicon.png";
import { Link } from "react-router-dom";

const ConsultantSidebar = ({ activeTab, onTabChange }) => {
  const navigate = useNavigate();
  const [open, setOpen] = useState(false);

  const menuItems = [
    { text: "Assigned Students", icon: <People />, tabIndex: 0 },
    { text: "Review Applications", icon: <Assignment />, tabIndex: 1 },
    { text: "Track Submissions", icon: <Timeline />, tabIndex: 2 },
    { text: "Chat", icon: <Chat />, tabIndex: 3 },
  ];
